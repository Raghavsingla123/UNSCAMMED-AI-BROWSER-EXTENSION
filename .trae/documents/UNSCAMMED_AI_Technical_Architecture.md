...
erDiagram
    ...
    EXTENSION_STATE ||--|| USER_SETTINGS : initialized_by
    EXTENSION_STATE ||--o{ URL_HISTORY : tracks
    URL_HISTORY ||--o{ SCAN_RESULT : references
```
...
```javascript
chrome.storage.local.get(['urlHistory'], ({ urlHistory = [] }) => {
  const history = [...urlHistory, urlLog].slice(-100);
