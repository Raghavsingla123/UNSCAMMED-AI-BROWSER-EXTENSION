...
// Google Web Risk Lookup API Client (URL Scanning)
// Uses: uris:search (FREE tier: 10k/month)
...
// Helper function to get API key (reads at runtime, not module load time)
function getAPIKey() {
  return process.env.PROJECT_B_API_KEY;
}
...
async function lookupUrl(url) {
  console.log(`[Lookup API] 🔍 Scanning URL: ${url}`);
